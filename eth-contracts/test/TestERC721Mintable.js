...
contract('TestERC721Mintable', accounts => {
    ...
    describe('match erc721 spec', function () {
        ...
        it('should return total supply', async function () {
          ...
          assert.equal(parseInt(amount),10,"Incorrect total supply count");
        })

        it('should get token balance', async function () {
          let balance = await this.contract.balanceOf(account_two);
          assert.equal(parseInt(balance), 10, "Incorrect token balance");
        })
        ...
        it('should transfer token from one owner to another', async function () {
          ...
          assert.equal(amount,account_three,"Token Transfer Unsuccessful");
        })
